...
This repository contains the code, configuration files, and preprocessed inputs (available on request) required to reproduce the analysis and results from the article "Functional Gradients of the Neuraxis: Reorganization from Cortex to Spinal Cord".

## 📑 Related paper:
Ekansh Sareen, Nivedya Nambiar, Boris Bernhardt et al. Functional Gradients of the Neuraxis: Reorganization from Cortex to Spinal Cord, 22 July 2026, PREPRINT (Version 1) available at Research Square [https://doi.org/10.21203/rs.3.rs-10155262/v1]
...
- `brain/`
  - `pipeline_brain.py`: cortical preprocessing (slice_timing, cropping image, motion correction, segmentation, registration, smoothing)
...
1. **Resting-state corticospinal fMRI preprocessing (optional if using provided timecourses)**  
   Use the scripts in `brsc_prpc/brain/`, `brsc_prpc/spine/`, and `brsc_prpc/denoise/`1together with the FSL templates and atlas files to preprocess raw fMRI data and extract parcel‑wise timecourses. 
